...
public:
    enum class Key {
        LeftMouse = 0,       ///< The left mouse button
        RightMouse = 1,      ///< The right mouse button
        MiddleMouse = 2,     ///< The middle (wheel) mouse button
        XMouse1 = 3,   ///< The first extra mouse button
        XMouse2 = 4,   ///< The second extra mouse button
	...
	static bool getKeyPressed(Key pKey);
    static bool getMousePressed(Key pMouseButton);

    static glm::ivec2 getMousePosition(sf::RenderWindow* pWindow);
private:
	Input();
	~Input();
};
